...
function _parseFaceTopology(data, vertEnd, vertexCount) {
    ...
    // Parse Block 2: slicing mask with sub-loop vertex counts
    const block1End = topoStart + (hN + 4) * 4;
    let loopVertexCounts = null;

    if (block1End + 16 <= data.length &&
        data.readUInt32LE(block1End) === 4 &&
        data.readUInt32LE(block1End + 4) === 8 &&
        data.readUInt32LE(block1End + 8) === 2) {
        const block2N = data.readUInt32LE(block1End + 12);
        if (block2N > 0 && block2N < 200) {
            loopVertexCounts = [];
            let totalVerts = 0;
            for (let t = 0; t < block2N; t++) {
                const raw = data.readUInt32LE(block1End + 16 + t * 4);
                const vc = (raw + 2) >> 1;
                if (vc >= 3) {
                    loopVertexCounts.push(vc);
                    totalVerts += vc;
                }
            }
            if (totalVerts !== vertexCount) loopVertexCounts = null;
        }
    }

    return { edgeIndices, topoStart, loopVertexCounts };
}
...
function _processPlaneFace(face, vertices, surfaceInfo, faceTopology) {
    ...
    // ================================================================
    // PATH B: Binary topology-driven loop splitting
    // ================================================================

    const topo = faceTopology;

    // If topology has loop vertex counts, use them to split the flat vertex array
    if (topo && topo.loopVertexCounts && topo.loopVertexCounts.length > 1) {
        const loops = [];
        let offset = 0;
        for (const vc of topo.loopVertexCounts) {
            if (offset + vc > n) break;
            const loop = [];
            for (let i = 0; i < vc; i++) loop.push(face[offset + i]);
    ...
    // Fallback: single-loop earcut
    const mergedVerts = [];
    for (let i = 0; i < n; i++) {
        const p = _proj3(pts3d[i]);
        mergedVerts.push(p[0], p[1]);
    }
    const triIndices = _earcut(mergedVerts, undefined, 2);
    if (triIndices && triIndices.length >= 3) {
        const tris = [];
        for (let i = 0; i < triIndices.length; i += 3) {
            tris.push([face[triIndices[i]], face[triIndices[i + 1]], face[triIndices[i + 2]]]);
        }
        if (tris.length > 0) return tris;
    }
