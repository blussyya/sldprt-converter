 ...
 *
 * Usage:
 *   node slprd-cli.js input.sldprt                    → input.obj
 *   node slprd-cli.js input.sldprt -o output.obj      → output.obj
 *   node slprd-cli.js input.sldprt -f stl             → input.stl
 *   node slprd-cli.js input.sldprt -f binary-stl      → input.stl (binary)
 *   node slprd-cli.js input.sldprt --scale 1000       → OBJ with coords ×1000
 *   node slprd-cli.js input.sldprt --info             → info only, no output
 *   node slprd-cli.js file1.sldprt file2.sldprt       → batch convert
 */
...
Usage:
  node slprd-cli.js <input.sldprt> [options]
...
Examples:
  node slprd-cli.js doneConsole.sldprt
  node slprd-cli.js doneConsole.sldprt -f stl --scale 1000
  node slprd-cli.js *.sldprt --info`);
}
...
function processFile(filePath, opts) {
    ...
    const outPath = opts.output || (path.dirname(absPath) + '/' + baseName + ext);
